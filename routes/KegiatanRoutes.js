...
router.post('/', kegiatan.single('gambar'), (req, res) => {
    ...
    const sql = ' INSERT INTO tbl_kegiatan (gambar,id_program,judul_kegiatan,deskripsi,tanggal_kegiatan) VALUES (?, ?, ?, ?, ?)';
    db.query(sql,[
        gambar,
        id_program,
        judul_kegiatan,
...
router.put('/:id_kegiatan', kegiatan.single('gambar'), (req,res) => {
    ...
    const sql = ' UPDATE tbl_kegiatan SET gambar = ?, id_program = ?, judul_kegiatan = ?, deskripsi = ?, tanggal_kegiatan = ? WHERE id_kegiatan = ?';
    db.query(
        sql,
        [
            gambar,
